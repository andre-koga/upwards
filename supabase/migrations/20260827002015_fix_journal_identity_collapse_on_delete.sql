-- Stop journal_entry upserts from being redirected onto the wrong row.
--
-- The previous by-date collapse ran `SELECT je.id ... WHERE je.entry_date = ?
-- LIMIT 1` with no ORDER BY and no deleted_at filter, then applied the payload
-- (including deleted_at) to whatever row came back. Two consequences:
--
--   1. A soft-delete op for a duplicate could land on the surviving entry and
--      mark real journal content deleted. The natural-id cutover enqueues
--      exactly such deletes, so upgrading devices silently lost entries.
--   2. Row choice was nondeterministic, so two devices could disagree about
--      which id is canonical for a date.
--
-- Fixes here:
--   * Only collapse onto a live (deleted_at IS NULL) row, so a tombstone can
--     never be resurrected into the canonical slot.
--   * Deterministic ORDER BY (exact id, then live, then oldest created_at, then
--     id) so every device agrees on the canonical row.
--   * Never redirect a delete: when the incoming payload carries deleted_at,
--     only ever apply it to that exact id. A delete for an id the server does
--     not know is a no-op rather than a delete of someone else's row.
--
-- Only the two journal_entry blocks differ from
-- 20260826235245_isolate_sync_op_apply_failures.sql.
