-- Optional 200-character note on activity periods.
-- Replaces submit_sync_operations so projection upserts persist the new column.
    ...
    IF status = 'accepted'
      ...
      IF entity_type NOT IN (
        ...
        IF entity_type = 'journal_entry' THEN
          SELECT je.updated_at::TEXT INTO remote_updated
          FROM journal_entries je
          WHERE je.id = entity_id AND je.user_id = uid;
    ...
    IF status = 'accepted'
      ...
      IF entity_type = 'journal_entry' THEN
        INSERT INTO journal_entries (
          ...
          entity_id, uid,
          NULLIF(projection_row->>'entry_date', '')::DATE,
          ...
          COALESCE(projection_row->'photo_paths', '[]'::jsonb),
        ...
        ON CONFLICT (id) DO UPDATE SET
          entry_date = COALESCE(EXCLUDED.entry_date, journal_entries.entry_date),
      ...
      ELSIF entity_type = 'activity_streak' THEN
        INSERT INTO activity_streaks (
          id, user_id, activity_id, date, streak, created_at, updated_at, deleted_at
        ) VALUES (
          entity_id, uid,
          NULLIF(projection_row->>'activity_id', '')::UUID,
          NULLIF(projection_row->>'date', '')::DATE,
          COALESCE(NULLIF(projection_row->>'streak', '')::INTEGER, 0),
          COALESCE(NULLIF(projection_row->>'created_at', '')::TIMESTAMPTZ, now()),
          COALESCE(NULLIF(projection_row->>'updated_at', '')::TIMESTAMPTZ, now()),
          NULLIF(projection_row->>'deleted_at', '')::TIMESTAMPTZ
        )
        ON CONFLICT (id) DO UPDATE SET
          activity_id = COALESCE(EXCLUDED.activity_id, activity_streaks.activity_id),
          date = COALESCE(EXCLUDED.date, activity_streaks.date),
