...
import { isNeverRoutine, isNeverTaskSlipRecorded } from "@/lib/activity/never-task";
import { isRoutineDueOnDate } from "@/lib/activity";
...
export function buildEntriesByDateMap(entries: DailyEntry[]): Map<string, DailyEntry> {
  ...
  while (cursor <= toDate) {
    ...
    if (breakDays.has(dateStr) && !isNever) {
      ...
      completionByDate[dateStr] =
        count >= (activity.completion_target ?? 1) ? "done" : "break";
    } else if (isRoutineDueOnDate(activity, cursor)) {
      const counts = entry?.task_counts as Record<string, number> | null;
      const count = counts?.[activity.id] ?? 0;
      if (isNever) {
        completionByDate[dateStr] = isNeverTaskSlipRecorded(activity, count)
          ? "slip"
          : "done";
      } else {
        completionByDate[dateStr] =
          count >= (activity.completion_target ?? 1) ? "done" : "missed";
      }
    } else {
...
export function completionRate(completed: number, scheduled: number): number | null {
